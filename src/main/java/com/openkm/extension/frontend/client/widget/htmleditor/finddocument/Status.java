...
public class Status extends PopupPanel {
	...
	public void refresh() {
		if (flag_getChilds) {
			int left = ((HTMLEditor.get().findDocumentSelectPopup.getOffsetWidth() - 200) / 2) +
					HTMLEditor.get().findDocumentSelectPopup.getAbsoluteLeft();
			int top = ((HTMLEditor.get().findDocumentSelectPopup.getOffsetHeight() - 40) / 2) +
					HTMLEditor.get().findDocumentSelectPopup.getAbsoluteTop();
			setPopupPosition(left, top);
			HTMLEditor.get().findDocumentSelectPopup.scrollDocumentPanel.addStyleName("okm-PanelRefreshing");
			super.show();
		} else {
			super.hide();
			HTMLEditor.get().findDocumentSelectPopup.scrollDocumentPanel.removeStyleName("okm-PanelRefreshing");
		}
	}
