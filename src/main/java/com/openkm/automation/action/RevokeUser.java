...
public class RevokeUser implements Action {
	...
	public void executePost(Map<String, Object> env, Object... params) throws Exception {
		String user = AutomationUtils.getString(0, params);
		Integer permissions = AutomationUtils.getInteger(1, params);
		String uuid = AutomationUtils.getUuid(env);

		if (uuid != null && user != null && !user.isEmpty()) {
			NodeBaseDAO.getInstance().revokeUserPermissions(uuid, user, permissions, false);
		}
	}
